...
gh repo create my-project-works --private --template jcosta33/suspec-starter-kit --clone   # or clone and re-init:
git clone https://github.com/jcosta33/suspec-starter-kit my-project-works && rm -rf my-project-works/.git && git -C my-project-works init

# or co-located inside your project
git clone https://github.com/jcosta33/suspec-starter-kit /tmp/kit && cp -R /tmp/kit/. your-repo/workspace/ && rm -rf your-repo/workspace/.git
...
.agents/skills/      the core guides — write-spec, implement-task, review-output — plus the
                     ...
                     spec-check, split-work, save-findings — plus the opt-in
                     implementation depth: write-feature, write-fix, write-refactor,
...
decisions/           your ADR ledger, seeded with 0001-adopt-suspec
status.md            the hand-edited workboard
hooks/               gate templates — a fail-open pre-commit hook and an authoritative CI
                     workflow that run `suspec check`; install them per hooks/README.md
...
1. Fill the `{{placeholders}}` — in `AGENTS.md` (Commands table, project facts) and in
   `decisions/0001-adopt-suspec.md` (date, team).
...
4. Wire the gate (recommended): install the pre-commit hook and copy the CI workflow per
   [`hooks/README.md`](hooks/README.md), so `suspec check` runs the [checks
   contract](https://github.com/jcosta33/suspec/blob/main/docs/reference/checks.md) on every
   commit and pull request. Needs [suspec-cli](https://github.com/jcosta33/suspec-cli).
...
[suspec-skills catalog](https://github.com/jcosta33/suspec-skills):
`npx skills add jcosta33/suspec-skills` (add `--list` to preview the catalog without installing).

Full instructions: [`docs/ADOPTING.md`](https://github.com/jcosta33/suspec/blob/main/docs/ADOPTING.md)
in the Suspec repo. Worked examples: [`docs/examples/`](https://github.com/jcosta33/suspec/tree/main/docs/examples).

The kit is [semver](https://semver.org)-versioned ([`VERSION`](./VERSION), [`CHANGELOG.md`](./CHANGELOG.md)) —
watch the [releases](https://github.com/jcosta33/suspec-starter-kit/releases) and re-copy the parts
you have not customized. If you use the optional CLI, `suspec update --check` tells you when your
copy is behind, and `suspec update --write` refreshes the kit-owned guidance for you (conflict-safe;
